...
t_vec3	phongLightning(t_rt *rt, t_hitRecord *hit_record)
{
	...
	return (vecMin(vecMultVec(light_color, rt->albedo), vec3(1, 1, 1)));
...
t_vec3	pointLightGet(t_rt *rt, t_elem *light, t_hitRecord *hit_record)
{
	...
	spec = pow(fmax(vecDotVec(view_dir, reflect_dir), 0.0), ksn);
	specular = vecMult(vecMult(light->color, ks), spec);
	specular = vecDivide(specular, 255);
	brightness = light->ratio * LUMEN; // 기준 광속/광량을 정의한 매크로
	return (vecMult(vecPlusVec(vecPlusVec(rt->ambient, diffuse), specular), brightness));
}

t_vec3	reflect(t_vec3 v, t_vec3 n)
{
	// v - 2 * dot(v, n) * n;
	return (vecMinusVec(v, vecMult(n, vecDotVec(v, n) * 2)));
}
