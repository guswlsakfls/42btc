...
/*   Updated: 2022/03/26 17:50:26 by hyujo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_parent_close(t_list *plines, t_pline *cur, t_pline *prev)
{
	if (prev && prev->pipe_fd[0]) // 이전 파이프 남아 있으면 닫자
		close(prev->pipe_fd[0]);
	if (cur->is_pipe == ISPIPE) // 여기 무한 루프 생겼음, 파이프 닫을 필요 없는데 닫아서 생김
		close(cur->pipe_fd[1]);
...
void	ft_parent(t_list *plines, t_pline *cur, t_pline *prev)
{
	...
	waitpid(cur->pid, NULL, WUNTRACED); // WUNTRACED 에러 발생했을 때 확인 // WNOHANG 하면 기다리지 않고 진행된다.
