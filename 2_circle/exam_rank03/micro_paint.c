...
int	get_info(FILE *file, drawing *drawing)
{
	...
	if (fscanf(file, "%d %d %c", &drawing->width, &drawing->height, &background) == 3)
	{
...
int	aplly_op(rectangle *rect, drawing *drawing)
{
	int	i;
	int	j;
...
int	main(int argc, char **argv)
{
	...
	if (argc == 2)
	{
		...
		wirte(1, "Error: Operation file corrupted\n", i);
	}
	else
	{
		i = ft_strlen("Error: argument\n");
		write(1, "Error: argument", i);
	}
	return (1);
}
